...
if [ "$1" = "healthcheck" ]; then
    case "$ROLE" in
        web) exec curl -fsS "http://127.0.0.1:${WEB_PORT:-80}/healthz" ;;
...
case "$ROLE" in
    collector)
        ...
        ;;
    bot)
        # Миграции накатывает именно bot — он один, гонки схемы не будет
    ...
    migrate)
        ...
        ;;
    *)
        echo "Неизвестная роль: ${ROLE}. Ожидается collector, bot, web или migrate." >&2
        exit 1
        ;;
esac
